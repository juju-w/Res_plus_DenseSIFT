...
class ResNet(object):
    ...
    def __init__(self, sess, args,i,j):
        ...
        if self.dataset_name == 'tiny' :
            ...
            self.c_dim = 3
            self.label_dim = 200
        if self.dataset_name == 'customize' :
            self.train_x, self.test_x,self.train_y,  self.test_y = load_customize(self.j)
            self.img_size = 48
            self.c_dim = 3
            self.label_dim = 2
        if self.dataset_name == 'sift':
            self.train_x, self.test_x, self.train_y, self.test_y = load_dsiftv2(self.j)
            self.img_size = 48
            self.c_dim = 3
            self.label_dim = 2
    ...
    def build_model(self):
        ...
        self.train_loss, self.train_accuracy,self.train_plab,self.train_tlab = classification_loss(logit=self.train_logits, label=self.train_labels)
        self.test_loss, self.test_accuracy,self.test_plab,self.test_tlab  = classification_loss(logit=self.test_logits, label=self.test_labels)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                # batch_x1 = self.test_x[idx*self.batch_size:(idx+1)*self.batch_size]
                # batch_y1 = self.test_y[idx*self.batch_size:(idx+1)*self.batch_size]
                #
                # batch_x1 = data_augmentation(batch_x1, self.img_size, self.dataset_name)
                # test_feed_dict = {
                #     self.test_inptus : batch_x1,
                #     self.test_labels : batch_y1
                test_feed_dict = {
                ...
                counter += 1
                print("Epoch: [%2d] [%5d/%5d] time: %4.4f, train_accuracy: %.5f, test_accuracy: %.5f, learning_rate : %.4f,train_loss: %5f" \
    ...
    def test(self):
        ...
        summary_str, test_loss, test_accuracy,p,t = self.sess.run(
            [self.test_summary, self.test_loss, self.test_accuracy,self.test_plab,self.test_tlab], feed_dict=test_feed_dict)
        import metrics
        print("test_accuracy: {}".format(test_accuracy))
        with open('resnet.txt', 'a') as f:  # 设置文件对象
            f.write(str(self.i) + '-' + str(self.j) + ',' + str(metrics.accuracy(t,p))+ ',' + str(metrics.precision(t,p))+ ',' + str(metrics.recall(t,p))+',' + str(metrics.f1score(t,p))
            +',' + str(metrics.ft(t,p))+ '\n')
    ...
    def fea_get(self):
        ...
        return np.array(subtrain).reshape(shape1[0]*shape1[2],shape1[-1]),np.array(self.train_y)[:,0],np.array(subtest).reshape(shape2[2],shape2[-1]),np.array(self.test_y)[:,0]
